...
class VectorStore:
    ...
    def add_unique_embeddings(self, embeddings_data: dict, source_text: str):
        ...
        unique_metadatas = []
        unique_ids = []
        for i, (embedding, document, metadata, id_) in enumerate(zip(embeddings, documents, metadatas, ids)):
            distance_to_chunk_in_collection = self.collection.query(
                query_texts=[document],
                n_results=1
            )["distances"][0]
            if not distance_to_chunk_in_collection or distance_to_chunk_in_collection[0] > 1e-3:
